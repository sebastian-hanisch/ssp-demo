...
st.caption(
    ...
    "Auf einem Netz mit Einheitskapazitäten zwischen Fahrzeugen und Aufträgen ist SSP die **Ungarische Methode** (Zuordnung mit Kosten: Demo \"Hungarian\" der Matching-Linie). Der umgekehrte Weg - zuerst irgendein zulässiger Fluss, dann negative Kreise löschen - ist **Cycle-Canceling**, noch nicht gebaut."
)
...
| **Jede Runde ist kostenminimal für ihre Menge** | Das ist die Stärke: SSP hält Optimalität und baut die Zulässigkeit (die gewünschte Menge) Schritt für Schritt auf. Man kann das auch umgekehrt tun: zuerst irgendein zulässiger Fluss, dann negative Kreise löschen. | **Cycle-Canceling** (nächstes Stück der Linie) |
| **Die Laufzeit hängt an der Menge** | SSP braucht höchstens so viele Runden wie der Flusswert (**pseudopolynomial**): bei Kapazitäten in Millionen kann es sehr viele Runden geben. Hier bleibt die Rundenzahl von der Größe der Kapazitäten unberührt. | **Cost Scaling** (Push-Relabel mit ε-optimalen Preisen), **Netzwerksimplex** (Demo „network-flow-demo“) |
...
st.caption("Die Netzwerkfluss-Linie ist als Ganzes geplant: Edmonds-Karp, Dinic, Push-Relabel, Successive Shortest Paths (dieses Stück), Cycle-Canceling, Cost Scaling, Mehrgüterfluss, Column Generation, Garg-Könemann, Fixkosten-Netzwerkdesign, Benders-Zerlegung und Slope Scaling - bisher sind die ersten vier gebaut.")
